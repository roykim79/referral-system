...
module.exports = app => {
  ...
  app.get("/api/referrals/sent", (req, res) => {
    ...
    Referral.find({referring_organization: req.user.organization.toHexString()}).populate("referring_organization").populate("receiving_organization").populate("referring_user").exec((err, data) => {
      if (err) {
        console.log(err);
        res.end();
      }
  ...
  app.get("/api/referrals/received", (req, res) => {
    ...
    Referral.find({receiving_organization: req.user.organization.toHexString()}).populate("referring_organization").populate("receiving_organization").populate("referring_user").exec((err, data) => {
      if (err) {
        console.log(err);
        res.end();
      }
  ...
  app.post("/api/referrals", async (req, res) => {
    ...
    await Organization.findOne({organizationName: req.body.receiving_organization}).exec((err, result) => {
  ...
  app.post("/api/referrals/:referralId/notes", (req, res) => {
    Referral.findById(req.params.referralId).populate('referring_organization').populate('receiving_organization').populate('referring_user').exec((err, referral) => {
      if (err) {
        console.log(err)
        res.end();
      } else {
        ...
        referral.tasks.push({posting_user: name, text: req.body.text});
  ...
  app.get("/api/referrals/:referralId", (req, res) => {
    Referral.findById(req.params.referralId).populate("referring_organization").populate("receiving_organization").populate("referring_user").exec((err, data) => {
      if (err) {
        console.log(err);
        res.end();
      }
